...
if (isset($_POST['submit'])) {
    ...
    if (empty($err)) {
        ...
        for ($i = 1; $i < count($sheetData); $i++) {
            $nama = $sheetData[$i]['0'];
            $mapel = $sheetData[$i]['1'];
            $kontak = $sheetData[$i]['2'];
            $status = $sheetData[$i]['3'];

            $d = query("SELECT gid FROM tb_guru WHERE gnama = '$nama' LIMIT 1")[0] ?? null;

            if ($d == null) {
                mysqli_query($conn, "INSERT INTO tb_guru VALUES (null, '$nama', '$mapel', '$kontak', '$status')");
                $jumlahDataBerhasil++;
            } else {
